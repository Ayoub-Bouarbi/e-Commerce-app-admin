  ...
  <div class="w-full overflow-x-hidden border-t flex flex-col">
    <main class="w-full flex-grow p-6">
      <div class="flex items-center justify-center">
        <h1 class="text-3xl flex-1 text-black">Attributes</h1>
        <nuxt-link :to="{name: 'attributes-create'}"
          ...
          <table class="min-w-full bg-white">
            <thead class="bg-gray-800 text-white">
              <tr>
                ...
                <th class="w-2/5 text-left py-3 px-4 uppercase font-semibold text-sm">Frontend Type</th>
            ...
            <tbody v-if="$apollo.queries.attributes.loading" class="text-gray-700">
              Loading...
            </tbody>
            <tbody v-else class="text-gray-700">
              <tr v-for="attribute of attributes" :key="attribute.id"
                ...
                <td class="w-3/5 text-left py-3 px-4"> {{ attribute.name }} </td>
                <td class="w-2/5 text-left py-3 px-4"> {{ attribute.frontend_type }} </td>
                <td class="w-66 text-center py-3 px-4">
                  <button class="px-1" @click="deleteAttribute(attribute)">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6" fill="none" viewBox="0 0 24 24" stroke="red">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                  </button>
                  <button class="px-1" @click="editAttribute(attribute)">
...
<script>
  import GET_ATTRIBUTE from '@/graphql/queries/attribute/attributes.gql';
  import DELETE_ATTRIBUTE from '@/graphql/mutations/attribute/deleteAttribute.gql';
        ...
        this.$swal({
          ...
          if (result.isConfirmed) {
            this.$apollo.mutate({
                ...
                let index = this.attributes.findIndex(c => c.id == attribute.id);
